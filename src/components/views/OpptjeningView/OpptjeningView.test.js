...
it("should render Opptjening view and display only the headings for the different panels", () => {
  ...
  expect(view.queryAllByRole("heading")[0]).toHaveTextContent(
    "beholdning-din-pensjonsbeholdning-i-folketrygden",
  ); //Beholdning-panel
  expect(view.queryAllByRole("heading")[1]).toHaveTextContent(
    "pensjonsbeholdning-forklart",
  ); //BeholdningForklart-panel
  expect(view.queryAllByRole("heading")[2]).toHaveTextContent(
    "inntekt-pensjonsgivende-inntekter",
  ); //Inntekter-panel
  expect(view.queryAllByRole("heading")[3]).toHaveTextContent(
    "chart-pensjonsbeholdningen-din",
  ); //Chart - Chart-view
  expect(view.queryAllByRole("heading")[4]).toHaveTextContent(
    "opptjening-details-din-okning-ar-for-ar",
  ); //OpptjeningDetails-panel
  expect(view.queryAllByRole("heading")[5]).toHaveTextContent(
    "opptjening-flere-steder-title",
  ); //OpptjeningFlereStederPanel
  expect(view.queryAllByRole("heading")[6]).toHaveTextContent(
    "pensjonskalkulator-lenke-title",
  ); //PensjonskalkulatorLenkePanel
  expect(view.queryAllByRole("heading")[7]).toHaveTextContent(
...
it("should render Opptjening view and display the headings for the different panels for users born between 1954 and 1962", () => {
  ...
  expect(view.queryAllByRole("heading")[1]).toHaveTextContent(
    "beholdning-and-pensjonspoeng-forklart",
  ); //BeholdningAndPensjonspoengForklart-panel
  expect(view.queryAllByRole("heading")[2]).toHaveTextContent(
    "beholdning-and-pensjonspoeng-forklart-pensjonspoeng-title",
  ); //Subtitle in BeholdningAndPensjonspoengForklart Panel
  expect(view.queryAllByRole("heading")[3]).toHaveTextContent(
    "beholdning-and-pensjonspoeng-forklart-pensjonsbeholdning-title",
  ); //Subtitle in BeholdningAndPensjonspoengForklart Panel
  expect(view.queryAllByRole("heading")[4]).toHaveTextContent(
    "inntekt-pensjonsgivende-inntekter",
  ); //Inntekter-panel
  expect(view.queryAllByRole("heading")[5]).toHaveTextContent(
    "chart-pensjonsbeholdningen-og-pensjonspoengene-dine",
  ); //Chart - Chart-view
  expect(view.queryAllByRole("heading")[6]).toHaveTextContent(
    "opptjening-details-din-okning-ar-for-ar",
  ); //OpptjeningDetails-panel
  ...
  expect(view.queryAllByRole("heading")[8]).toHaveTextContent(
    "pensjonskalkulator-lenke-title",
  ); //PensjonskalkulatorLenkePanel
  expect(view.queryAllByRole("heading")[9]).toHaveTextContent(
    "faq-ofte-stilte-sporsmaal",
  ); //FAQ-panel
...
it("should render Opptjening view and display the headings for the different panels for users born before 1954", () => {
  ...
  expect(view.queryAllByRole("heading")[1]).toHaveTextContent(
    "inntekt-pensjonsgivende-inntekter-og-pensjonspoeng",
  ); //Inntekter-panel
  expect(view.queryAllByRole("heading")[2]).toHaveTextContent(
    "opptjening-flere-steder-title",
  ); //OpptjeningFlereStederPanel
...
it("should render Opptjening view, open details panel, and show correct beholdning for selected year", () => {
  ...
  expect(
    view.getByTestId("amount-opptjening-details-din-pensjonsbeholdning")
      .textContent,
  ).toEqual(formatAmount(expectedBeholdning2000));
  ...
  expect(
    view.getByTestId("amount-opptjening-details-din-pensjonsbeholdning")
      .textContent,
  ).toEqual(formatAmount(expectedBeholdning2001));
});
...
it("should render Opptjening view with OverforeOmsorgsOpptjening link panel present", () => {
  ...
  const mockState3 = mockStateFromOpptjeningData(2012, [
    constructOpptjening({
      merknader: ["OMSORGSOPPTJENING", "OVERFORE_OMSORGSOPPTJENING"],
    }),
  ]);
  ...
  expect(view.queryAllByRole("heading")[5]).toHaveTextContent(
    "overfore-omsorgsopptjening-title",
  ); //OverforeOmsorgsOpptjening-panel
  expect(view2.queryAllByRole("heading")[5]).toHaveTextContent(
    "overfore-omsorgsopptjening-title",
  ); //OverforeOmsorgsOpptjening-panel
  expect(view3.queryAllByRole("heading")[5]).toHaveTextContent(
    "overfore-omsorgsopptjening-title",
  ); //OverforeOmsorgsOpptjening-panel
});

it("should Opptjening view, open details panel and select year", () => {
  const mockStore = configureStore();
  let store = mockStore(mockedState);
  ...
  expect(view.getByTestId("year-selector")).toHaveValue("2000");
});
...
it("should render Opptjening view and display the Veileder with speechbubble containg the correct name and text", () => {
  ...
  expect(view.getByTestId("veilederContainer")).toBeVisible(); //Veileder component
  expect(view.getByTestId("veilederContainer")).toHaveTextContent(
    "opptjening-hei, test tester testesen! opptjening-intro-tekst",
  );
});
