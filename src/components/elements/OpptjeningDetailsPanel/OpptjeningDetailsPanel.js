...
const detailRow = (props) => {
  return (
    <div
      role="row"
      data-testid={props.key}
      key={props.key}
      className="detailRow"
    >
      <span
        role="cell"
        data-testid={"label-" + props.key}
        className="labelColumn"
      >
        {props.label}
      </span>
      <span role="cell" data-testid={"kr-" + props.key} className="krColumn">
        kr
      </span>
      <span
        role="cell"
        data-testid={"amount-" + props.key}
        className="numberColumn"
      >
        {props.amount}
      </span>
      <span role="presentation" className="emptyColumn">
        &nbsp;
      </span>
    </div>
  );
...
const getTextParagraph = (text, key) => {
  return (
    <p key={key} data-testid={key} className="typo-normal">
      {text}
    </p>
  );
};
...
const getLabelForGrunnlagCode = (grunnlagCode, grunnlag, t) => {
  switch (grunnlagCode) {
    case "INNTEKT_GRUNNLAG":
      return t(
        "opptjening-details-opptjening-basert-paa-pensjonsgivende-inntekt",
        { grunnlag },
      );
    case "UFORE_GRUNNLAG":
      return t("opptjening-details-opptjening-basert-paa-uforetrygd", {
        grunnlag,
      });
    case "FORSTEGANGSTJENESTE_GRUNNLAG":
      return t("opptjening-details-opptjening-basert-paa-forstegangstjeneste", {
        grunnlag,
      });
    case "DAGPENGER_GRUNNLAG":
      return t("opptjening-details-opptjening-basert-paa-dagpenger", {
        grunnlag,
      });
...
const buildDetails = (
  opptjening,
  currentYear,
  hasOmsorgsOpptjeningTwoYearsBack,
  t,
) => {
  let details = [];
    ...
    opptjening.endringOpptjening.forEach((endring, idx) => {
      ...
      switch (endring.arsakType) {
        case "INNGAENDE":
          ...
          break;
        case "INNGAENDE_2010":
          grunnlagTextArray.push(
            t("opptjening-details-okning-pga-reform-merknad"),
          );
          item = detailRow({
            key: "detail-" + idx,
        ...
        case "OPPTJENING":
          ...
          if (endring.grunnlagTypes.length === 1) {
            const grunnlagType = t(
              "grunnlag:" + endring.grunnlagTypes[0] + "_TYPE",
            );
            label = getLabelForGrunnlagCode(
              endring.grunnlagTypes[0],
              grunnlag,
              t,
            );
            grunnlagTextArray.push(
              t("grunnlag:" + endring.grunnlagTypes[0] + "_DESCRIPTION", {
                uforegrad,
                year: currentYear - 2,
              }),
            );
            grunnlagTextArray.push(
              t("opptjening-details-lurer-du-paa-se-ofte-stilte-spm", {
            ...
            const grunnlagTypesString = grunnlagTypes
              .join(", ")
              .replace(/,([^,]*)$/, " " + ogString + "$1");
            label = t(
              "opptjening-details-opptjening-basert-paa-flere-ytelser",
              { grunnlagTypes: grunnlagTypesString, grunnlag },
            );
            grunnlagTextArray.push(
              t("opptjening-details-lurer-du-paa-se-ofte-stilte-spm", {
                grunnlagType: grunnlagTypesString,
              }),
            );
          }
          ...
          if (
            hasOmsorgsOpptjeningTwoYearsBack &&
            !endring.grunnlagTypes.includes("OMSORGSOPPTJENING_GRUNNLAG")
          ) {
            grunnlagTextArray.push(
              t("opptjening-details-omsorgsopptjening-text", {
                year: currentYear - 2,
              }),
            );
          }
        ...
        case "UTTAK":
          if (
            endring.endringBelop !== 0 &&
            Math.abs(endring.endringBelop) !== currentRegulering
          ) {
            item = detailRow({
              key: "detail-" + idx,
              label: t("opptjening-details-uttak", {
                uttaksgrad: endring.uttaksgrad,
              }),
              amount: formatAmount(endring.endringBelop),
            });
  ...
  return {
    detailRows: details,
    grunnlagTexts: getTextParagraphsFromTextArray(
      grunnlagTextArray,
      "grunnlagtext",
    ),
    uttakTexts: getTextParagraph(
      Object.values(uttakTexts).join(" "),
      "uttaktext",
    ),
  };
};
    ...
    <div className="detailsBox">
      ...
      <div className="detailRow">
        <span className="labelColumn">
          {pensjonspoeng != null
            ? t("opptjening-details-pensjonspoeng-label", { currentYear })
            : t("opptjening-details-ingen-pensjonspoeng")}
        </span>
        {pensjonspoeng != null && (
          <span
            data-testid="pensjonspoengContainer-pensjonspoeng"
            className="numberColumn"
          >
            {formatNumber(pensjonspoeng)}
          </span>
        )}
        <span role="presentation" className="emptyColumn">
          &nbsp;
...
export const OpptjeningDetailsPanel = (props) => {
  ...
  const hasOmsorgsOpptjeningTwoYearsBack =
    props.hasOmsorgsOpptjeningTwoYearsBack;

  let { detailRows, grunnlagTexts, uttakTexts } = buildDetails(
    opptjening,
    currentYear,
    hasOmsorgsOpptjeningTwoYearsBack,
    t,
  ...
  const explanationTextsContainer = getExplanationTextsContainer(
    grunnlagTexts,
    uttakTexts,
  );
  const pensjonspoengContainer = getPensjonspoengContainer(
    opptjening.pensjonspoeng,
    currentYear,
    t,
  );
  ...
  if (opptjening.pensjonsbeholdning !== null) {
    ...
    detailRows = (
      <div role="row">
        <span role="cell">
          {t("opptjening-details-ingen-pensjonsbeholdning", {
            year: currentYear - 2,
          })}
        </span>
      </div>
  ...
  const panelTitle = (
    <PanelTitle
      id="opptjeningDetailsTitle"
      titleString={t("opptjening-details-din-okning-ar-for-ar")}
      illustrationClass="detailIllustration"
      illustration={sedler}
    />
  );
  ...
  return (
    ...
    >
      <div className="yearSelectorContainer">
        <h3>
          <Label htmlFor="yearSelector" className="label">
            {t("opptjening-details-velg-ar")}
          </Label>
        </h3>
        <div className="selectorWrapper">
          <YearSelector
            years={props.yearArray}
