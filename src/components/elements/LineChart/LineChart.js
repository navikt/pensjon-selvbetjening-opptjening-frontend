...
const amountRow = (amount, t) => {
  if (amount === null) {
    return <div>{t("chart-ingen")}</div>;
  } else {
    return <div aria-label={amount}>{formatAmount(amount)}</div>;
  }
};

const amountListItem = (amount, t) => {
  if (amount === null) {
    return <span>{t("chart-ingen")}</span>;
  } else {
    return <span>kr {formatAmount(amount)}</span>;
  }
};
  ...
  return (
    <tr key={key} className={"tableRow " + rowClass}>
      ...
      {userGroup === BORN_IN_OR_BETWEEN_1954_AND_1962 && (
        <td data-testid="tableDataPensjonspoeng">
          {pensjonspoeng != null
  ...
  return (
    <li className={"beholdningPoengItem " + rowClass} key={key}>
      <ul>
        ...
        <li>
          {t("chart-pensjonsbeholdning") + ": "} {pensjonsbeholdningTxt}
        </li>
        {userGroup === BORN_IN_OR_BETWEEN_1954_AND_1962 && (
          <li>
            {t("chart-pensjonspoeng") + ": "}{" "}
            {pensjonspoeng !== null
              ? formatNumber(pensjonspoeng)
              : t("chart-ingen")}
          </li>
...
const getUttakArray = (data) => {
  ...
  if (data.uttak.length > 0) {
    uttakArray = data.uttak.map((uttak) => {
      ...
      const month = dato.toLocaleDateString(getCurrentLocale(), {
        month: "long",
      });
...
export const LineChart = (props) => {
  ...
  const chartConfig = {
    ...
    options: {
      ...
      onClick: function (event, item) {
        if (item && item[0]) {
          props.onclick
            ? props.onclick(Object.keys(chartMap)[item[0]._index])
            : emptyFn();
        }
      },
      legend: {
      ...
      tooltips: {
        callbacks: {
          ...
          label: function (tooltipItem, data) {
            ...
            let tooltip = [];
            const tooltipBeholdning =
              tableMap[year].pensjonsbeholdning == null
                ? t("chart-ingen")
            ...
            if (userGroup === BORN_IN_OR_BETWEEN_1954_AND_1962) {
              let poeng = formatNumber(tableMap[year].pensjonspoeng)
                ? formatNumber(tableMap[year].pensjonspoeng)
                : t("chart-ingen");
              tooltip.push("");
  ...
  const { dataRows, dataListItems } = buildDataForTableAndListView(
    tableMap,
    userGroup,
    showAll,
    t,
  ...
  const ChartKnapp = (props) => {
    return (
      <Knapp
        id="grafknapp"
        mini
        className={chartButton + " " + props.className}
        onClick={() => toggleVisibleComponent("chart")}
      >
        {t("chart-graf")}
      </Knapp>
    );
  };
  const TableKnapp = (props) => {
    return (
      <Knapp
        id="tabellknapp"
        mini
        className={tableButton + " " + props.className}
        onClick={() => toggleVisibleComponent("table")}
      >
        {t("chart-tabell")}
      </Knapp>
    );
  ...
  return (
    <div id="beholdningAndPoengTabellTop" tabIndex="-1">
      ...
      {userGroup === BORN_IN_OR_BETWEEN_1954_AND_1962 && (
        <p>
          {t("chart-antall-aar-med-pensjonspoeng", {
          ...
          <table className="tabell beholdningAndPoengTabell">
            <thead>
              <tr className="tableRow">
                <th data-testid="tableHeaderYear" className="column1">
                  {yearLabel}
                </th>
                <th
                  data-testid="tableHeaderPensjonsbeholdning"
                  className="column2"
                >
                  {pensjonsbeholdningKrLabel}
                </th>
                {userGroup === BORN_IN_OR_BETWEEN_1954_AND_1962 && (
                  <th
                    data-testid="tableHeaderPensjonspoeng"
                    className="column3"
                  >
                    {pensjonspoengLabel}
                  </th>
                )}
                <th data-testid="tableHeaderUttak" className="column4">
                  {uttakLabel}
                </th>
              </tr>
            </thead>
            <tbody>{dataRows.reverse()}</tbody>
          </table>
          <ul className="beholdningAndPoengList">{dataListItems.reverse()}</ul>
          <div className="expandTableLink">
            {showAll && (
              <Lenke
                href="#beholdningAndPoengTabellTop"
                onClick={(event) => toggleShowAll(event, !showAll)}
              >
                <OppChevron />
                <br />
            ...
            {!showAll && (
              <Lenke
                id="tabell-vis-alle-knapp"
                href="#"
                onClick={(event) => toggleShowAll(event, !showAll)}
              >
                {t("chart-vis-alle-aar")}
                <br />
                <NedChevron />
              </Lenke>
            )}
