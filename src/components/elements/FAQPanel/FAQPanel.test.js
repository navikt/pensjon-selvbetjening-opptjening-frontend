...
import {BORN_AFTER_1962, BORN_BEFORE_1943, BORN_IN_OR_BETWEEN_1943_AND_1953} from "../../../common/userGroups";
...
it('should render FaqView with special questions for 1943_AND_1953 and common questions for users born before 1943', () => {
    const { getByTestId, queryAllByRole } = render(<FAQPanel userGroup={BORN_BEFORE_1943}/>);
    expect(getByTestId("faqview")).toBeVisible();
    expect(queryAllByRole("button")[0]).toHaveTextContent("faq_born_in_or_between_1943_and_1953:faq-question-1");
    expect(queryAllByRole("button")[1]).toHaveTextContent("faq:faq-question-1");
    expect(queryAllByRole("button")[2]).toHaveTextContent("faq:faq-question-2");
