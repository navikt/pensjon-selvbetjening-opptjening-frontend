...
export const FAQPanel = (props) => {
  ...
  for (let i = 1; i <= numberOfSpecialQuestions; i++) {
    ...
    faq.push(
      <Ekspanderbartpanel
        key={"special" + i}
        tittel={
          <Normaltekst>{t(faqNamespace + ":faq-question-" + i)}</Normaltekst>
        }
        border
        className="questionWrapper"
        onClick={() => toggleOpen(i, "special")}
      >
        <div key="horizontalLine" className="faqHorizontalLine" />
        <ReactMarkdown linkTarget="blank">
          {t(faqNamespace + ":faq-answer-" + i, { joinArrays: "\n\n" })}
        </ReactMarkdown>
      </Ekspanderbartpanel>,
    );
  }
  for (let i = 1; i <= numberOfCommonQuestions; i++) {
    ...
    faq.push(
      <Ekspanderbartpanel
        key={"common" + i}
        tittel={<Normaltekst>{t("faq:faq-question-" + i)}</Normaltekst>}
        border
        className="questionWrapper"
        onClick={() => toggleOpen(i, "common")}
      >
        <div key="horizontalLine" className="faqHorizontalLine" />
        <ReactMarkdown linkTarget="blank">
          {t("faq:faq-answer-" + i, { joinArrays: "\n\n" })}
        </ReactMarkdown>
      </Ekspanderbartpanel>,
    );
  }
  ...
  return (
    <Panel border className="panelWrapper">
      <div className="faqTitleContainer">
        <PanelTitle
          id="faqTitle"
          titleString={t("faq-ofte-stilte-sporsmaal")}
          illustrationClass="faqIllustration"
          illustration={sporsmalstegn}
        />
      </div>
      <div data-testid="faqview">{faq}</div>
    </Panel>
  );
};
