...
const mockData = mockStateFromOpptjeningData(startYear, [
...
it("should not fail any accessibility tests", async () => {
  const { getByRole, container } = render(
    <InntektWithMerknadPanel data={mockData} userGroup={BORN_AFTER_1962} />,
  );
...
it("should render the closed Inntekt panel with correct heading", () => {
  ...
  expect(panel.getByRole("heading")).toHaveTextContent(
...
it("should render the Inntekt panel and open it with correct mockData sorted", () => {
  ...
  expect(panel.getAllByTestId("income-header")[0]).toHaveTextContent(
    "inntekt-aar",
  );
  expect(panel.getAllByTestId("income-header")[1]).toHaveTextContent(
    "inntekt-inntekt-kr",
  );
  expect(panel.getAllByTestId("income-header")[2]).toHaveTextContent(
    "inntekt-merknad",
  );

  expect(panel.getAllByTestId("income-row").length).toBe(4);

  expect(panel.getAllByTestId("income-label")[1]).toHaveTextContent(
    startYear + 2,
  );
  expect(panel.getAllByTestId("income-amount")[1].textContent).toEqual(
    formatAmount(expectedInntekt2020),
  );
  expect(panel.getAllByTestId("income-label")[2]).toHaveTextContent(
    startYear + 1,
  );
  expect(panel.getAllByTestId("income-amount")[2].textContent).toEqual(
    formatAmount(expectedInntekt2019),
  );
  expect(panel.getAllByTestId("income-label")[3]).toHaveTextContent(startYear);
  expect(panel.getAllByTestId("income-amount")[3].textContent).toEqual(
    formatAmount(expectedInntekt2018),
  );
});
...
it("should render text about opplysninger pa senere tidspunkt when inntekt is null", () => {
  ...
  expect(panel.getAllByTestId("income-label")[0]).toHaveTextContent(
    expectedYear,
  );
  expect(panel.getAllByTestId("income-amount")[0]).toHaveTextContent(
...
it("should render the InntektWithMerknadPanel with pensjonspoeng when user BORN_IN_OR_BETWEEN_1943_AND_1953", () => {
  ...
  expect(getAllByTestId("income-header")[2]).toHaveTextContent(
    "inntekt-pensjonspoeng",
  );
});
...
it("should not render the InntektWithMerknadPanel with pensjonspoeng when user BORN_IN_OR_BETWEEN_1954_AND_1962", () => {
  ...
  expect(getAllByTestId("income-header")[2]).not.toHaveTextContent(
    "inntekt-pensjonspoeng",
  );
});
...
it("should show antall år med pensjonspoeng for usergroup BORN_IN_OR_BETWEEN_1943_AND_1953", () => {
  ...
  render(
    <InntektWithMerknadPanel
      data={{}}
      userGroup={BORN_IN_OR_BETWEEN_1943_AND_1953}
      antallAarPensjonsPoeng={expectedAntallAarMedPensjonsPoeng}
    />,
  );

  userEvent.click(screen.getByRole("heading"));
  expect(
    screen.getByText("inntekt-antall-aar-med-pensjonspoeng"),
  ).toBeInTheDocument();
});
...
it("should not show antall år med pensjonspoeng for usergroup BORN_IN_OR_BETWEEN_1954_AND_1962", () => {
  ...
  userEvent.click(screen.getByRole("heading"));
  expect(
    screen.queryByText("inntekt-antall-aar-med-pensjonspoeng"),
