...
import {InntektWithMerknadPanel} from './InntektPanel';
...
it('should not fail any accessibility tests', async () => {
    const {getByRole, container} = render(<InntektWithMerknadPanel data={mockData}/>);
...
it('should render the closed Inntekt panel with correct heading', () => {
    ...
    const panel = render(<InntektWithMerknadPanel data={mockData}/>);
...
it('should render text about opplysninger pa senere tidspunkt when inntekt is null', () => {
    ...
    const panel = render(<InntektWithMerknadPanel data={inntektState}/>);
    expect(panel.getByRole("heading")).toHaveTextContent("opptjening-pensjonsgivende-inntekter");
