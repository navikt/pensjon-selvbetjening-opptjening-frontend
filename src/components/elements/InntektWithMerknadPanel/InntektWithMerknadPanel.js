...
import "./InntektWithMerknadPanel.css";
...
const getMerknadListItem = (text, key) => {
  return text !== null && <li key={key}>{text}</li>;
};
...
const getMerknadText = (merknad, t, uforegrad) => {
  const overforeUrl = urlHelper.OVERFORE_OMSORGSOPPTJENING_URL
    ? urlHelper.OVERFORE_OMSORGSOPPTJENING_URL
    : "";
...
const detailRow = (props) => {
  ...
  merknader.forEach((m, idx) => {
    merknadArray.push(
      getTextParagraph(getMerknadText(m, t, uforegrad), "remarkstext-" + idx),
    );
  });
  ...
  return (
    <tr data-testid="income-row" key={key} className="tableRow">
      ...
      {userGroup === BORN_IN_OR_BETWEEN_1943_AND_1953 && (
        <td data-testid="pensjonspoeng">
          {pensjonsPoeng !== null ? formatNumber(pensjonsPoeng) : null}
        </td>
      )}
...
const detailListItem = (props) => {
  ...
  merknader.forEach((m, idx) => {
    const merknadListItem = getMerknadListItem(
      getMerknadText(m, t, uforegrad),
      "remarkstext-" + idx,
    );
    if (merknadListItem) {
  ...
  return (
    <li className="inntektItem" key={key}>
      <ul>
        ...
        <li>
          {t("inntekt-inntekt") + ":"} {amountTxt}
        </li>
        {userGroup === BORN_IN_OR_BETWEEN_1943_AND_1953 && (
          <li>
            {t("inntekt-pensjonspoeng") + ":"}{" "}
            {pensjonsPoeng !== null ? formatNumber(pensjonsPoeng) : null}
          </li>
        )}
        {merknadArray.length > 0 && (
          <li className="listItemMerknad">
            {t("inntekt-merknader") + ":"} <ul>{merknadArray}</ul>
          </li>
        )}
      </ul>
    </li>
...
const buildDetails = (opptjeningData, userGroup, t) => {
  ...
  Object.keys(opptjeningData).forEach((year, idx) => {
    const detailProps = {
      ...
      year: year,
      amount:
        opptjeningData[year].pensjonsgivendeInntekt !== null
          ? formatAmount(opptjeningData[year].pensjonsgivendeInntekt)
          : null,
      explanationText: t(
        "opptjening-opplysningen-vil-komme-pa-et-senere-tidspunkt",
      ),
  ...
  return {
    detailRows: detailRows.reverse(),
    detailListItems: detailListItems.reverse(),
  };
};
...
export const InntektWithMerknadPanel = (props) => {
  ...
  const panelTitle = (
    <PanelTitle
      titleString={t(title)}
      illustrationClass="inntektDetailIllustration"
      illustration={handMedMynter}
    />
  );
    ...
    <div>
      ...
      >
        <div data-testid="inntektContainer">
          ...
          {userGroup === BORN_IN_OR_BETWEEN_1943_AND_1953 && (
            <p>
              {t("inntekt-antall-aar-med-pensjonspoeng", {
                years: antallAarPensjonsPoeng,
              })}
            </p>
          )}
          <div className="inntektDetailsBox">
            <table className="tabell inntektTabell">
              <thead>
                <tr className="tableRow">
                  <th data-testid="income-header" className="col1">
                    {t("inntekt-aar")}
                  </th>
                  <th data-testid="income-header" className="col2">
                    {t("inntekt-inntekt-kr")}
                  </th>
                  {userGroup === BORN_IN_OR_BETWEEN_1943_AND_1953 && (
                    <th data-testid="income-header" className="col3">
                      {t("inntekt-pensjonspoeng")}
                    </th>
                  )}
                  <th data-testid="income-header" className="col4">
                    {t("inntekt-merknad")}
                  </th>
                </tr>
              </thead>
            ...
            <ul className="inntektList">{detailListItems}</ul>
          </div>
          <button
            type="button"
            aria-label={t("inntekt-til-toppen")}
